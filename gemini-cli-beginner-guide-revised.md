...
4. Git 설치 확인
5. im-not-ai 확장 기능 설치
6. Gemini CLI에서 im-not-ai 사용
...
Gemini CLI 공식 문서의 권장 운영체제는 macOS 15 이상입니다. 이전 버전의 macOS는 공식 권장 환경 밖이므로, 설치 오류가 나면 먼저 macOS 버전과 업데이트 가능 여부를 확인하세요.
...
이 메시지는 npm의 전역 설치 위치에 현재 사용자가 쓸 권한이 없다는 뜻입니다. **터미널을 다시 열거나 같은 명령어를 반복해도 권한 문제는 해결되지 않습니다. 또한 설치 명령어 앞에 무작정 sudo를 붙이지 마세요.**

Homebrew가 이미 설치된 Mac이라면 다음 명령어로 확인할 수 있습니다.

~~~bash
brew --version
~~~

버전 숫자가 나오면 npm 명령 대신 Gemini CLI 공식 문서에서 제공하는 다음 설치 방법을 사용할 수 있습니다.
...
`brew: command not found`가 나온다면 Homebrew도 설치되어 있지 않은 것입니다. 이 경우에는 명령어를 임의로 더 입력하지 말고 [npm의 EACCES 공식 해결 문서](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally/)를 참고해 Node.js를 버전 관리자로 다시 설치하거나, 컴퓨터에 익숙한 사람에게 도움을 받는 편이 안전합니다.
...
**Git은 GitHub에 있는 확장 기능을 내려받을 때 필요한 프로그램입니다.** Gemini CLI 공식 문서에서도 GitHub 주소로 확장 기능을 설치하려면 Git이 설치되어 있어야 한다고 안내합니다.

일반 터미널에서 다음 명령어를 입력합니다.

~~~bash
git --version
~~~

다음처럼 버전 숫자가 보이면 Git이 준비된 것입니다. 숫자는 달라도 괜찮습니다.

~~~text
git version 2.39.5
~~~

처음 사용하는 Mac에서는 개발자 명령어 도구를 설치하라는 창이 나타날 수 있습니다. 이 창이 나타나면 다음 순서로 진행하세요.

1. Install 또는 설치를 누릅니다.
2. 사용권 계약이 나오면 내용을 확인하고 동의합니다.
3. 설치가 끝날 때까지 기다립니다.
4. 터미널을 닫았다가 다시 엽니다.
5. `git --version`을 다시 입력해 버전 숫자가 나오는지 확인합니다.

설치 창이 나타나지 않으면서 Git을 찾을 수 없다고 나오면 다음 명령어로 macOS의 설치 창을 열 수 있습니다.

~~~bash
xcode-select --install
~~~

설치를 마친 뒤 터미널을 새로 열고 `git --version`을 다시 확인하세요.
...
명령어를 실행하기 전에 입력한 주소가 다음 주소와 같은지 확인하세요.

~~~text
https://github.com/epoko77-ai/im-not-ai.git
~~~

설치 중에 외부 확장 기능의 보안 위험을 확인하거나 설치에 동의하는 질문이 나타날 수 있습니다. 설치하려는 대상이 `epoko77-ai/im-not-ai`가 맞다면 Yes, 동의 또는 계속에 해당하는 항목을 선택합니다. 다른 저장소나 예상하지 못한 대상이 표시되면 설치를 취소하고 명령어를 다시 확인하세요.
...
3. 다음 명령어를 입력해 Git 버전이 나오는지 확인합니다.

~~~bash
git --version
~~~

4. Git을 찾을 수 없다고 나오면 앞의 `4-2. Git 설치 확인` 단계로 돌아갑니다.
5. Git이 정상이라면 인터넷 연결을 확인합니다.
6. 1~2분 뒤 같은 설치 명령어를 다시 시도합니다.
...
5. /commands list로 humanize-korean 명령이 보이는지 확인합니다.
...
전용 명령어를 사용할 때는 **명령어와 윤문할 글을 같은 입력에 넣고, 모두 붙여넣은 다음 마지막에 Enter를 누릅니다.** `/humanize-korean`만 입력하고 먼저 Enter를 누르지 마세요.

다음 블록 전체를 복사한 뒤, `[윤문할 글]` 부분을 실제 글로 바꿔 한 번에 입력하면 됩니다.
...
짧은 글은 `/humanize-korean` 뒤에 공백을 하나 넣고 같은 줄에 바로 이어서 입력해도 됩니다.
...
3. Gemini 입력창에 다음 명령어를 입력하되, 아직 Enter를 누르지 않기

~~~text
/humanize-korean
~~~

4. 명령어 뒤에 공백을 하나 넣고 윤문할 글을 같은 입력창에 붙여넣은 다음 Enter 누르기
...
Node.js, npm, Git이 이미 설치되어 있다는 전제에서 일반 터미널에 입력할 명령어는 다음 두 줄입니다.
...
Gemini 화면에서 명령어 뒤에 윤문할 글을 함께 입력합니다.
...
정상이라면 각각 Node.js 버전 숫자, npm 버전 숫자, Git 버전 숫자, Gemini CLI 버전 숫자, 설치된 확장 기능 목록이 나옵니다.
...
- [Gemini CLI 확장 기능 상세 문서](https://geminicli.com/docs/extensions/reference/)
- [npm EACCES 오류 해결 공식 문서](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally/)
- [Apple 명령어 도구 설치 공식 문서](https://developer.apple.com/documentation/xcode/installing-the-command-line-tools)
